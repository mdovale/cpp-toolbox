...
Chapters 1 through 8 (`build-and-ub`, `raii-and-ownership`,
`cmake-app`, `floating-point`, `error-handling`,
`small-linear-algebra`, `frames-and-rotations`, `ode-rk4`) are
written. Later chapters remain planned until they exist as the same
kind of slice, including `prose: complete`.
